...
MAD_K = 2.5              # <- TU VALOR ORIGINAL (más cercano a la realidad)
MIN_CONSECUTIVOS = 1
...
def detect_peaks(df, target_col, mad_k=2.5, min_consec=1):
...
def smooth_series(df, target_col, method="cap"):
    # "cap": reemplaza por upper_cap (umbral). "med": por mediana del (dow,hour).
    if method == "cap":
...
def build_feature_matrix_nn(df, target_col, include_peak_flag=True):
    ...
    if include_peak_flag and "is_peak" in df.columns:
        base_feats.append("is_peak")  # <- NUEVA FEATURE
...
def main():
    ...
    # Features de tiempo y rolling
    df = add_time_features(df)
    df = rolling_features(df, COL_LLAMADAS)

    # Detección de picos + suavizado (MAD_K=3.5, SUAVIZADO="cap")
    ...
    # Export de picos para verificación
    ...
    # Entrenamiento LLAMADAS (usa la serie suavizada y la bandera is_peak)
    df_train_ll = df_pk.copy()
    df_train_ll[COL_LLAMADAS] = df_train_ll[COL_LLAMADAS + "_smooth"]

    print("\n--- Preparando Modelo de LLAMADAS ---")
    X_ll = build_feature_matrix_nn(df_train_ll, COL_LLAMADAS, include_peak_flag=True)
    y_ll = df_train_ll.loc[X_ll.index, COL_LLAMADAS].astype(float)

    Xtr_ll, Xte_ll, ytr_ll, yte_ll = train_test_split(
        X_ll, y_ll, test_size=TEST_SIZE, shuffle=False, random_state=RANDOM_STATE
    )

    scaler_llamadas = StandardScaler()
    # Importante: fit con DataFrame (con columnas) para guardar feature_names_in_
    ...
    # Entrenamiento TMO (usa también is_peak como feature)
    print("\n--- Preparando Modelo de TMO ---")
    df_tmo = df_pk.copy()
    df_tmo = rolling_features(df_tmo, "tmo_seg")
    X_tmo = build_feature_matrix_nn(df_tmo, "tmo_seg", include_peak_flag=True)
    y_tmo = df_tmo.loc[X_tmo.index, "tmo_seg"].astype(float)
    ...
    if len(y_tmo) > 100:
        Xtr_t, Xte_t, ytr_t, yte_t = train_test_split(
            X_tmo, y_tmo, test_size=TEST_SIZE, shuffle=False, random_state=RANDOM_STATE
        )
        ...
        model_tmo, scaler_tmo = None, None
        print(f"AVISO: TMO no entrenado (muestras válidas={len(y_tmo)}).")
